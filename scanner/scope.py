...
class TargetScope:
    def __init__(
        ...
        raise_on_dns_failure: bool = False,
    ):
        self.allow_local = (
            allow_local
            or os.environ.get("ALLOW_LOCAL_TARGETS", "").lower()
            in ("1", "true", "yes")
        )
        ...
        if (
            not self.parsed.scheme
            or self.parsed.scheme.lower() not in ("http", "https")
        ):
            raise TargetInvalidSchemeError(
                f"Invalid URL scheme '{self.parsed.scheme}'. "
                "Only HTTP and HTTPS are permitted."
            )

        if not self.parsed.hostname:
            raise ScopeViolationError(
                "Target URL must specify a valid hostname."
            )
    ...
    def _validate_ssrf(self, hostname: str):
        """
        Validates that hostname does not resolve to loopback, private,
        link-local, multicast, reserved, or cloud-metadata addresses.

        NAT64 handling:
        Addresses in 64:ff9b::/96 are IPv4-embedded translation addresses.
        Their embedded IPv4 address is extracted and passed through the same
        SSRF safety checks. Therefore a NAT64 address pointing to a private,
        loopback, link-local, or metadata IPv4 address is still blocked.
        """
        ...
        if (
            not self.allow_local
            and (
                clean_host in banned_hosts
                or clean_host.endswith(".localhost")
                or clean_host.endswith(".internal")
            )
        ):
            raise TargetSSRFBlockedError(
                f"SSRF Protection: Access to '{hostname}' is strictly blocked."
            )
        ...
        try:
            ...
            for item in addr_info:
                ...
                # 64:ff9b::67bf:d05c
                #
                # The last 32 bits contain the translated IPv4 address.
                # Validate that embedded IPv4 address rather than rejecting
                # the NAT64 translation prefix itself as "reserved".
                if (
                    isinstance(ip_obj, ipaddress.IPv6Address)
                    and ip_obj in ipaddress.IPv6Network("64:ff9b::/96")
                ):
                    embedded_ipv4 = ipaddress.IPv4Address(
                        int(ip_obj) & 0xFFFFFFFF
                    )

                    if not self.resolved_ipv4:
                        self.resolved_ipv4 = str(embedded_ipv4)

                    if not self.resolved_ipv6:
                        self.resolved_ipv6 = str(ip_obj)

                    if not self.allow_local:
                        self._check_ip_safety(embedded_ipv4)

                    continue

                if (
                    isinstance(ip_obj, ipaddress.IPv4Address)
                    and not self.resolved_ipv4
                ):
                    self.resolved_ipv4 = str(ip_obj)

                elif (
                    isinstance(ip_obj, ipaddress.IPv6Address)
                    and not self.resolved_ipv6
                ):
                    self.resolved_ipv6 = str(ip_obj)
            ...
            if self.raise_on_dns_failure:
                raise TargetUnavailableError(
                    f"DNS Resolution Failed: Unable to resolve hostname "
                    f"'{hostname}': {e}"
                )
    ...
    def _check_ip_safety(
        ...
        if (
            ...
            raise TargetSSRFBlockedError(
                f"SSRF Protection: Target IP address '{ip}' is in a private, "
                "loopback, or reserved range."
            )

        # Specific check for AWS / GCP / Azure metadata address
        # 169.254.169.254
        if str(ip) == "169.254.169.254":
            raise TargetSSRFBlockedError(
                "SSRF Protection: Cloud metadata IP "
                "(169.254.169.254) is strictly prohibited."
            )
    ...
    def validate_candidate_url(self, candidate_url: str) -> None:
        ...
        if cand_scheme not in ("http", "https"):
            raise TargetInvalidSchemeError(
                f"Unsafe URL scheme '{cand_scheme}'. "
                "Only HTTP and HTTPS are permitted."
            )
        ...
        if not cand_host:
            raise ScopeViolationError(
                "Candidate URL lacks a valid hostname."
            )
    ...
    def is_same_origin(self, candidate_url: str) -> bool:
        ...
        try:
            ...
            cand_port = parsed.port or (
                443 if cand_scheme == "https" else 80
            )
            ...
            # Allow scheme if matching or upgrade from http to https
            is_http_upgrade = (
                self.scheme == "http"
            ...
            if cand_scheme != self.scheme and not (
                self.scheme == "http" and cand_scheme == "https"
            ):
                return False
    ...
    def normalize_url(
        ...
        """
        Resolves relative URLs to absolute, removes fragments,
        and normalizes path.
        """
        if base_url:

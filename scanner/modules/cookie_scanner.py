"""
SentinelScan - Cookie Security Scanner
...
class CookieScanner:
    def scan(self, pages: List[CrawledPage]) -> List[Dict[str, Any]]:
        ...
        for page in pages:
            # Look for Set-Cookie header (could be multiple in raw headers)
            # In httpx dict, Set-Cookie might be single or joined with commas or case-insensitive
            set_cookie_raw = None
            ...
            for cookie_str in raw_cookies:
                ...
                sanitized_cookie_repr = f"{cookie_name}=[REDACTED_VALUE]; {sanitized_flags}".strip("; ")

                # 1. Secure flag check
                if "secure" not in flags and is_https:
                    findings.append(create_finding(
                        scanner="cookie_scanner",

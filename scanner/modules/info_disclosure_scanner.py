"""
SentinelScan - Information Disclosure Scanner
Safely checks for:
    ...
    def scan(self, scope: TargetScope, pages: List[CrawledPage]) -> List[Dict[str, Any]]:
        ...
        for page in pages:
            ...
            if server and server not in seen_servers:
                ...
                # Flag if it exposes version numbers (e.g. Apache/2.4.41 or nginx/1.18.0)
        ...
        try:
            with httpx.Client(verify=not scope.allow_local, timeout=5.0) as client:
                ...
                # Check for standard env patterns safely without capturing values
                if env_resp.status_code == 200:
                    text_head = env_resp.text[:300]
                    # Check if it looks like an environment file
